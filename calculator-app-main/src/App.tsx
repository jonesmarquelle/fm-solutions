...
function App() {
  ...
  const buttons: Key[] = [7, 8, 9, DELETE, 4, 5, 6, "+", 1, 2, 3, "-", ".", 0, "/", "×"].map((v) => {
      ...
      <div id='bg' className={themes[theme]}>
        <div  id='calculator'>
              <div className={`title-bar`}>
                ...
                <NumberedSlider ticks={3} defaultVal={theme} onSliderChange={(v) => {setTheme(v)}}/>
              </div>
              ...
              <div className='button-grid'>
                {buttons.map((key) => 
                  <button className={key.value === DELETE ? "del-button" : "button"} key={key.text} onClick={() => key.f(key.value)}>
                    {key.text}
                  </button>
                )}
                <button className='reset-button' onClick={resetCalc}>RESET</button>
                <button className='equal-button' onClick={doEquals}>=</button>
              </div>  
        </div>
      </div>
    </>
